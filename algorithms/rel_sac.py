...
class AttentionSAC(object):
    ...
    def __init__(self, agent_init_params, sa_size,
                 ...
                 **kwargs):
        # TODO does not seem like more parameters are added
        """
        Inputs:
...
class RelationalSAC(object):
    ...
    def __init__(self,
        ...
        self.agents = [AttentionAgent(lr=pi_lr,
                         ...
                         for params in agent_init_params] # are input and output dims for agent
        self.critic = RelationalCritic(obj_n=params['obj_n'],
                                       n_actions=n_actions,
                                       input_dims=input_dims,
                                       hidden_dim=critic_hidden_dim) # TODO
        self.target_critic = RelationalCritic(sa_size, hidden_dim=critic_hidden_dim) # TODO
        hard_update(self.target_critic, self.critic)
        self.critic_optimizer = Adam(self.critic.parameters(), lr=q_lr,
                                     weight_decay=1e-3)
        self.agent_init_params = agent_init_params #in: obs.shape out: action.shape
    ...
    def init_from_env(cls, env, gamma=0.95, tau=0.01,
        ...
        for acsp, obsp in zip(env.action_space,
                              env.observation_space):
            agent_init_params.append({'num_in_pol': obsp['image'].shape, # TODO flatten?
                                      'num_out_pol': acsp.n})
            sa_size.append((obsp['image'].shape, acsp.n))
            s_size.append(obsp['image'].shape)
            a_size.append(ascp.n)
        ...
        init_dict = {'gamma': gamma,
                     ...
                     'agent_init_params': agent_init_params,
                     'sa_size': sa_size,
                     'n_obj': env.nb_all_entities,
                     'n_actions': a_size,
                     's_size': s_size,
                     'input_dims': [env.obs_shape[0][0],env.obs_shape[0][1][-1],env.obs_shape[2][-1] ],
                     }
        instance = cls(**init_dict)
        instance.init_dict = init_dict
        return instance
