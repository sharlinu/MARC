...
def run():
    ...
    if default_config.alg == 'MARC':
        ...
        env = make_parallel_MAAC_env(default_config,seed=1)
        env.grid_observation = default_config.grid_observation
        env.reset()
        model = AttentionSAC.init_from_env(env,
                                           ...
                                           reward_scale=default_config.reward_scale)
        replay_buffer = ReplayBufferMAAC(default_config.maac['buffer_length'], model.nagents,
            ...
            if (len(replay_buffer) >= hyper_config.batch_size and
                ...
                for u_i in range(default_config.num_updates):
                    sample = replay_buffer.sample(hyper_config.batch_size,
                                                  to_gpu=default_config.use_gpu, norm_rews=default_config.norm_rews)
...
def make_env(default_config):
    if default_config.env_name == 'lbf':
        from lbforaging.foraging import ForagingEnv
        env = ForagingEnv(
            players=default_config.player,
            max_player_level=default_config.lbf['max_player_level'],
            field_size=(default_config.field, default_config.field),
            max_food=default_config.lbf['max_food'],
            grid_observation=default_config.grid_observation,
            sight=default_config.field,
            max_episode_steps=default_config.episode_length,
            force_coop=default_config.lbf['force_coop'],
            keep_food=default_config.lbf['keep_food'],
            # simple=default_config.simple,
        )
    elif default_config.env_name == 'bpush':
        from bpush.environment import BoulderPush
        env = BoulderPush(
            width=default_config.field,
            height=default_config.field,
            n_agents=default_config.player,
            penalty=default_config.bpush['penalty'],
            sensor_range=default_config.bpush['sensor_range'],
        )
        if default_config.alg == 'MARC':
        ...
        env = Wolfpack(
            ...
            num_players=default_config.player,
            max_food_num=default_config.wolfpack['max_food_num'],
            obs_type=default_config.wolfpack['obs_type'],
            close_penalty = default_config.wolfpack['close_penalty'],
            sparse = default_config.wolfpack['sparse'],
        )

        env.n_agents = env.num_players
        env.grid_observation = True
    else:
        raise ValueError(f'No known env {default_config.env_name} ')
    return env
...
if __name__ == '__main__':
    sweep_config = {
        ...
        "parameters": {
            ...
            "pi_lr": {'values':[0.0001, 0.0005, 0.001]},
            "q_lr": {'values':[0.0001, 0.0005, 0.001]},
